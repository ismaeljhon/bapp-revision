        ...
        onSubmit() {
            this.$validator.validateAll().then(noerrors => {
                if (noerrors) {
                    ...
                    return new RestApiService('/portal/' + process.env.VUE_APP_PORTAL_ID + "/projects/" + this.item.id + "/tasks/").save({ params: this.form }, true)
                        .then(response => {
                            ...
                            this.isLoading = false;
                            Log.error(error, { processType: 'response', customMessage: "Error on creating new task" })
                        })
                }
            })
        }
