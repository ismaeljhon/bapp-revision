...
import { join as _join, assign as _assign, find as _find } from "lodash";
import swal from "sweetalert";
import Log from "@/shared/Log";
import { getCurrentZohoProjectUser } from "@/mixins/modules/User";
...
let Timelog = {
  methods: {
    recordTimelog(rawData) {
      ...
      Log.info("Record Timelog: " + JSON.stringify(rawData), {
        processType: "process",
      });
      return true;
    },
    pushTimelog(rawData) {
      ...
      let notes =
        "timesheet log : start time -" +
        localStorage.ZOHO_TIME_STARTED +
        " end time -" +
      ...
      Log.info("Pushing Timelog " + JSON.stringify(rawData), {
        processType: "request",
      });

      return new RestApiService(
        "/portal/" +
          process.env.VUE_APP_PORTAL_ID +
          "/projects/" +
          rawData.project_id +
          "/tasks/" +
          taskId +
        ...
        .catch((error) => {
          Log.error(error, {
            processType: "response",
            customMessage: "Error on pushing your Timelog",
          });
        });
    },
    getTimelogData(data) {
    ...
    getPendingTimelog() {
      ...
      if (currentUser.id == pendingTimelogs.owner) {
        swal({
          ...
          closeOnEsc: false,
          text:
            pendingTimelogs.project.name +
            "\n\n" +
            pendingTimelogs.task.name +
            "\n\n" +
            "Time Spent: " +
            pendingTimelogs.hours,
          icon: "warning",
          buttons: {
            do_not_push: {
              text: "Do not Push",
    ...
    fetchWeeklyTimelogs(overrideOptions = {}) {
      let params = {
        users_list: getCurrentZohoProjectUser().id,
        view_type: overrideOptions.view_type
          ? overrideOptions.view_type
          : "week",
        date: overrideOptions.date
          ? overrideOptions.date
      ...
      return new RestApiService(
        "/portal/" + process.env.VUE_APP_PORTAL_ID + "/logs"
      )
        .index(params)
        .then((response) => {
          ...
          localStorage.setItem(
            "ZOHO_WEEKLY_TIMELOGS",
            JSON.stringify(timelogs)
          );
          this.$store.commit("SET_WEEKLY_TIMELOGS", timelogs);

          Log.info(
            "Weekly Timelog successfully fetch " + JSON.stringify(timelogs),
            { processType: "response" }
          );
        })
        .catch((error) => {
          Log.error(error, {
            processType: "response",
            customMessage: "Error on fetching weekly timelog",
          });
        });
    },
    getWeeklyTimelogs() {
    ...
    fetchDailyTimelogs() {
      ...
      return new RestApiService(
        ...
        .then((response) => {
          ...
          Log.info(
            "Daily Timelog successfully fetch " + JSON.stringify(timelogs),
            { processType: "response" }
          );
        })
        .catch((error) => {
